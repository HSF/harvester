...
class SshMasterPool(object):
    ...
    def make_control_master(self, remote_host, remote_port, num_masters=1,
        ...
        # preserve parameters
        if not reconnect:
            self.params[dict_key] = {'num_masters': num_masters,
        ...
        for i in range(num_masters - len(self.pool[dict_key])):
            ...
            com = com.format(remote_host=remote_host, remote_port=remote_port,
                             ssh_username=ssh_username, private_key=private_key, jump_host=jump_host,
                             jump_port=jump_port, sock_file=sock_file)
            ...
            for iTry in range(3):
                ...
                if idx == 1:
                    # timeout
                    baseLogger.error('timeout when making a master with com={0} out={1}'.format(com,
                                                                                                c.buffer))
                    c.close()
                    break
                if idx == 2:
