...
def make_ssh_connection(ssh_host, ssh_port, ssh_username, ssh_password, pass_phrase, private_key,
                        jump_host, jump_port):
    # ssh
    ...
    if jump_host is None:
        # direct SSH
        sshClient.connect(ssh_host, ssh_port, username=ssh_username, password=ssh_password,
                          passphrase=pass_phrase, key_filename=private_key)
    else:
        # via jump host
        sshClient.connect(jump_host, jump_port, username=ssh_username, password=ssh_password,
                          passphrase=pass_phrase, key_filename=private_key)
        ...
        jumpClient.connect(ssh_host, ssh_port, username=ssh_username, password=ssh_password,
                           passphrase=pass_phrase, key_filename=private_key, sock=channel)
    return sshClient
