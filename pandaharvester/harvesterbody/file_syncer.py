...
class FileSyncer(AgentBase):
            ...
            try:
                ...
                for setup_name, setup_map in setup_maps.items():
                    try:
                        ...
                        _logger.error(f"failed to launch file_syncer in pluginConfigs for {plugin_params}")
    ...
    def execute(self):
        ...
        for exe_core in itertools.chain(self.exe_cores, self.queue_exe_cores):
            ...
            mainLog = self.make_logger(_logger, f"{exe_core.__class__.__name__} {file_syncer_name}", method_name="execute")
            try:
                # check freshness
                mainLog.debug("check")
