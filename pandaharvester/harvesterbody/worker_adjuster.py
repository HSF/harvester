...
class WorkerAdjuster(object):
    ...
    def _dict_to_dataframe(self, static_num_workers: dict) -> pl.DataFrame:
        ...
        Input dict structure:
            {queue_name: {job_type: {resource_type: {nQueue, nReady, nRunning, nNewWorkers}}}}
        ...
        for queue_name, queue_dict in static_num_workers.items():
            for job_type, rt_dict in queue_dict.items():
                for resource_type, stats in rt_dict.items():
                    rows.append(
                        {
                            "job_type": job_type,
    ...
    def define_num_workers(self, static_num_workers, site_name) -> dict | None:
        ...
        try:
            ...
            for queue_name in static_num_workers:
                ...
                # Single loop over dataframe rows - replaces nested for-loops
                for row_idx, row in enumerate(df_queue.iter_rows(named=True)):
                    job_type = row["job_type"]
                    resource_type = row["resource_type"]
                    prod_source_label = row["prod_source_label"]
                    n_queue = row["nQueue"]
                    ...
                    if queue_name in queue_stat and queue_stat[queue_name]["status"] in ["offline", "standby", "maintenance"]:
                        df_queue = df_queue.with_columns(pl.when(pl.col("job_type") == job_type).then(0).otherwise(pl.col("nNewWorkers")).alias("nNewWorkers"))
                    ...
                    if throttler is not None:
                        ...
                        if to_throttle:
                            df_queue = df_queue.with_columns(
                                pl.when(pl.col("job_type") == job_type).then(0).otherwise(pl.col("nNewWorkers")).alias("nNewWorkers")
                            )
                    ...
                    if n_queue >= n_queue_limit_per_rt > 0:
                        ...
                        ret_msg = f"No n_new_workers since n_queue({n_queue}) + n_ready({n_ready}) + n_running({n_running}) >= max_workers({max_workers})"
                        tmp_log.debug(ret_msg)
                        pass
                    ...
                    df_queue = df_queue.with_columns(
                        pl.when(
                            (pl.col("job_type") == job_type) & (pl.col("resource_type") == resource_type) & (pl.col("prod_source_label") == prod_source_label)
                        )
                        .then(n_new_workers)
                        .otherwise(pl.col("nNewWorkers"))
                        .alias("nNewWorkers")
