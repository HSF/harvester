...
class CommandManager(AgentBase):
    ...
    def run(self):
        ...
        main
        """
        main_log = self.make_logger(_logger, f"id={self.get_pid()}", method_name="run")
        ...
        while True:
            ...
            if locked or self.singleMode:
                ...
                while continuous_loop:
                    ...
                    main_log.debug(f"got {len(commands)} commands (bulk size: {bulk_size})")
                    ...
                    main_log.debug(f"cached {len(command_specs)} commands in internal DB")
                    ...
                    for shard in core_utils.create_shards(command_ids_ack, bulk_size):
                        ...
                        main_log.debug(f"acknowledged {len(shard)} commands to panda server")
