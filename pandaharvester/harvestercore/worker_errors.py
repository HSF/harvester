...
# 4. CE errors (shown in condor HoldReason)
CMPM_CE = {
...
# 6. Errors of Special Compute Resource Manager (K8s, etc.)

# 6.8 Kubernetes errors
CMPM_Kubernetes = {
    6801: r"Job has reached the specified backoff limit",
    6810: r"Pod was rejected: Node didn't have enough resource: .*",
    6814: r"Pod was rejected: .*",
    6815: r"Pod was active on the node longer than the specified deadline",
    6819: r"Pod was .*",
    # 6.8.8 Harvester K8s plugin reserved errors
    6881: r"Failed to get status for id=.*",
    6882: r"JOB id=.* not found",
    6883: r"container not terminated yet \(.*\) while pod Succeeded",
    6884: r"container terminated by k8s for reason .*",
}

# 8. Kill or remove messages
CMPM_Kill_Remove = {
...
class WorkerErrors(object):
    ...
    # HTCondor plugin worker error message patterns
    htcondor_message_pattern_handler = ErrorMessagePatternHandler(
        CMPM_Pilot_Wrapper | CMPM_HTCondor_Schedd | CMPM_CE | CMPM_Batch_System | CMPM_Kill_Remove | CMPM_Catchall
    )

    # Kubernetes plugin worker error message patterns
    k8s_message_pattern_handler = ErrorMessagePatternHandler(CMPM_Kubernetes | CMPM_Catchall)
