...
for item in aux.items:
    try:
        if item.status.container_statuses[0].state.waiting.reason == 'ContainerCreating' and current_time - item.metadata.creation_timestamp > datetime.timedelta(minutes=30):
